...
import RetornoEldoc from './retornoEldoc'
...
class NFeEldoc extends Component {
    constructor(props) {
        super(props)
        ...
        this.state = {
            docuEldoc: undefined,
            documentoRetornoEldoc: undefined,
            nfe: undefined,
    ...
    componentDidMount() {
        const docuEldoc = this.props.location.docuEldoc;
        //this.getDocumentoRetornoEldoc(docuEldoc);
        this.getNfe(docuEldoc);
    }

    getDocumentoRetornoEldoc(docuEldoc) {
        const idDocFiscal = "idDocFiscal=" + docuEldoc.id;
        const url = `${config.endpoint['fazemu-nfe']}/documentoRetornoEldoc?${idDocFiscal}`;

        this.setState({ loading: true })
        axiosAuth.get(url, getConfig())
            .then(response => {
                this.setState({
                    documentoRetornoEldoc: response.data
                })
            }).catch(error => {
    ...
    getNfe(docuEldoc) {
        const url = `${config.endpoint['fazemu-nfe']}/documentoFiscalEldoc/eldoc/nfe/${docuEldoc.chaveAcesso}`;
    ...
    render() {
        ...
        const documentoRetornoEldoc = pathOr([], ['documentoRetornoEldoc'], this.state)
            ...
            <div>
                ...
                <div className="container-fluid px-5">
                    <div>
                        ...
                        {this.renderButtons(docuEldoc, documentoRetornoEldoc)}
                        {this.renderTabs(docuEldoc, nfe)}
    ...
    renderButtons(docuEldoc, documentoRetornoEldoc) {
        const rowsRetorno = [];
        documentoRetornoEldoc.map((documentoRetorno, key) => {
            rowsRetorno.push(
                <RetornoEldoc key={key} documentoRetorno={documentoRetorno} />
            )
        })
        ...
        return (
            <div className="mt-1">
                <div className="btn-group" role="group">
                    <Link to="/nfeEldocInit"><Button color="danger" className="btn btn-secondary mr-1" size="sm">Voltar</Button></Link>
                ...
                <div>
                    <div className="mt-1">
                        ...
                        <TabContent activeTab={this.state.activeTab}>
                            {this.renderSituacaoDocumento(docuEldoc)}
    ...
    renderSituacaoDocumento(docuEldoc) {
        return (
                ...
                <div className="card mt-1">
                    ...
                    <div className="card-body">
                        <div className="row">
                            ...
                            <span className="col-sm-9">{docuEldoc.chaveAcessoEnviada}</span>
                        </div>
                        <div className="row">
                            <b className="col-sm-2">Data de Recebimento</b>
                            <span className="col-sm-9">{docuEldoc.dataHoraReg}</span>
                        </div>
                        <div className="row">
                            <b className="col-sm-2">Tipo Emissão</b>
                            <span className="col-sm-9">{TipoEmissaoEnum[docuEldoc.tipoEmissao]}</span>
                        </div>
                        <div className="row">
                            <b className="col-sm-2">Código do Retorno</b>
                            <span className="col-sm-9">{docuEldoc.situacaoAutorizador}</span>
                        </div>
                    </div>
                </div>
            </TabPane>
