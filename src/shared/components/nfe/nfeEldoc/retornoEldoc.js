...
class RetornoEldoc extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getXml = (documentoRetorno) => {
        ...
        const url = `${config.endpoint['fazemu-nfe']}/documentoRetornoEldoc/xml?${idXml}`;
                ...
                <span>
                    <div className="btn-group" role="group">
                        <a href={this.getXml(documentoRetorno)} target="_blank">
                            <Button className="btn btn-secondary mr-1" size="sm"> {TipoServicoEnum[documentoRetorno.tipoServico]} {TipoEventoManifestacaoEnum[documentoRetorno.tipoEvento]}</Button>
                        </a>
                    </div>
                </span>
            );
